...
if __name__ == "__main__":
    # 指定单个PLY文件的路径
    single_ply_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("data/daqu/daqu.ply")
    
    # 输出目录设置为文件所在的目录
    target_out_dir = single_ply_path.parent
    
    # 直接调用函数处理该文件
    rasterize_ply(single_ply_path, target_out_dir)
