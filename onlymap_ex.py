...
for t in range(5):
    if args.mode == 'eval':
        ...
        if args.landmark_mode == 'predictor':
            ...
            # 导航至 landmark
            trajectory_logs, _, _ = run_episodes_batch(args, model, test_episodes, DEVICE, landmark_mode='predictor')
        ...
        for episode in test_episodes:
            # 创建Agent实例
            agent = SceneAgent(args, episode.start_pose, episode, vlmodel, set_height=None)
            # 设置目标
        ...
        with open(f'map_controller_{args.split}_{args.progress_stop_val}{noise}{alt_env}_{args.eval_goal_selector}_{t}.json', 'w') as f:
            json.dump({
