...
def run_episodes_batch(
    ...
    if landmark_mode == 'predictor':
        for episodes_batch in tqdm(dataloader, desc='eval episodes', unit='batch', colour='#88dd88', position=1):
            ...
            for t in trange(args.eval_max_timestep, desc='eval timestep', unit='step', colour='#66aa66', position=2, leave=False):
                ...
                # predict
                pred_normalized_goal_xys, pred_progresses = model(maps, rgbs, normalized_depths, flip_depth=True)
    ...
    #使用基于地标（landmark）的规划器直接导航到预设地标
        ...
        for t in trange(args.eval_max_timestep, desc='move timestep', unit='step', colour='#66aa66', position=2, leave=False):
            gps_noise_batch = np.random.normal(scale=args.gps_noise_scale, size=(len(poses), 2))
            noisy_poses = [Pose4D(x + n_x, y + n_y, z, yaw) for (x, y, z, yaw), (n_x, n_y) in zip(poses, gps_noise_batch)]

            poses = [
                move(pose, xy, args.move_iteration, noisy_pose)
                for pose, noisy_pose, xy in zip(poses, noisy_poses, target_xys)
            ]
            # 记录轨迹
            for eps, pose, noisy_pose, done in zip(episodes, poses, noisy_poses, dones):
                if not done:
                    pose_logs[eps.id].append(pose)

            if all(pose.xy.dist_to(xy) < args.success_dist for pose, xy in zip(poses, target_xys)):
                break
        return dict(pose_logs), dict(target_xys)
